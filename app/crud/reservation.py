...
class CRUDReservation(CRUDBase[MODEL, ReservationCreate]):
    ...
    def create_reservation(
        ...
        if not room.status or room.room_availability != "available":
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "Room is not available for booking")
        ...
        if not self.check_room_availability(db, room.id, check_in_dt, check_out_dt):
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "Room is already booked for the selected dates")
        ...
        if not room_type:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "Room has no room type configured")

        nights = (check_out_dt.date() - check_in_dt.date()).days
        deposit_amount = round(room_type.price_per_night * nights * room_type.deposit_percentage / 100)
    ...
    def get_guest_reservations(self, db: Session, guest_id: int, page: int = 1, limit: int = 10):
        return self.read(
            db,
            page=page,
            limit=limit,
    ...
    # ------------------------------------------------------------
    def update_reservation(self, db: Session, uid: UUID, record_in: ReservationUpdate) -> Reservations:
        reservation = self.get_reservation(db, uid)

        if reservation.status in ("checked_out", "cancelled", "expired"):
            raise HTTPException(status.HTTP_400_BAD_REQUEST, f"Cannot modify a {reservation.status} reservation")

        needs_recheck = record_in.room_id or record_in.check_in_date or record_in.check_out_date
        if needs_recheck:
            ...
            if not self.check_room_availability(
                db, new_room_id, new_check_in, new_check_out, exclude_reservation_id=reservation.id
            ):
                raise HTTPException(status.HTTP_400_BAD_REQUEST, "Room is already booked for the selected dates")
    ...
    # status lifecycle
    # ------------------------------------------------------------
    def _transition(self, db: Session, uid: UUID, allowed_from: tuple, new_status: str) -> Reservations:
        reservation = self.get_reservation(db, uid)
    ...
    def cancel_reservation(self, db: Session, uid: UUID) -> Reservations:
        return self._transition(db, uid, ("pending", "confirmed"), "cancelled")
